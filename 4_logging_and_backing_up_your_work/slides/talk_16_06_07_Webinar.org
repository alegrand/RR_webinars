...
*** Problems with the Standard Approaches
#+LaTeX: \vspace{-.3cm}
****                                                     :B_ignoreheading:
:PROPERTIES:
...
- doc+email+revisions: 
  ...
  + Never sure what is the latest version
...
- Multiple people editing the same file simulataneously
- Everyone commits locally, occasionally merging to remote
- If editing different paragraphs
  $\leadsto$ merging is automatic
- If editing same paragraphs
  $\leadsto$ need to handle conflicts
- At the end clean history of the article writing, with all intermediary versions

*** Working with Textual Files
- Necessary so Git can make differences for commits
- Common solutions: LaTeX, markdown, Org-mode, etc.
- Many user-friendly environments that make LaTeX usage today much easier
- No dependencies on (proprietary) software tools
- Alternative is to convert binaries into text files:
...
- Side note: those of you who still collaborate with SVN users may enjoy =git svn=
...
**** Major Challenges
  - Large files and thus repositories (=git lfs=, =git annex=, etc.)
  - Linear history hard to explore and thus exploit
*** git xp (demo)
#+BEGIN_CENTER
\includegraphics[width=.6\linewidth]{figures/gitxp.pdf}
#+END_CENTER
...
*** git xp (conclusion)
- Unique branching system that unites all phases of the research process
- Eases reproduction of the experiments and the replicability of the analysis

#+BEGIN_CENTER
maybe modified author/reader figure highlighting different parts, linking them to our different types of branches?
#+END_CENTER

*** Additional Challenge: Integrating several Git Sources
...
- [[https://github.com/arfon/fidgit][GitHub+figshare]]: similar approach from the same author
